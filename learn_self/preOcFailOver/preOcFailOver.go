...
func main()  {
	...
	//vipList(vip, vPort)
	err := rsAddDel(vip, vPort, "del")
	if err != nil {

		sendMsg := fmt.Sprintf("【故障转移之前阶段】\n故障类型：'%s'\n故障详细描述：'%s'\n实例类型：'%s'\n故障集群 ：'%s'\n故障集群别名：'%s'\n故障集群域名：'%s'\n故障实例：'%s:%s'\n新主库：'%s:%s'\n具体操作：移除vip %s:%s 后端RS %s:%s失败\n失败原因：%s",
			failureType,failureDescription,instanceType,failureCluster, failureClusterAlias,failureClusterDomain,failedHost,failedPort,successorHost,successorPort,vip,vPort,failedHost,failedPort,err)
		sendDcMsg(sendMsg)
		// 返回非0的状态值
		exitWithError(fmt.Errorf("移除vip %s:%s 后端RS %s:%s失败：%s",vip,vPort,failedHost,failedPort,err))
	}
