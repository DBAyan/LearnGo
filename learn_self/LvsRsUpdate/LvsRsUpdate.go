...
func main(){
	...
	sendMsg := fmt.Sprintf("【切换失败通知】\n故障类型：'%s'\n故障详细描述：'%s'\n实例类型：'%s'\n故障集群 ：'%s'\n故障集群别名：'%s'\n故障集群域名：'%s'\n故障实例：'%s:%s'\n新主库：'%s:%s'\n",
		failureType,failureDescription,instanceType,failureCluster, failureClusterAlias,failureClusterDomain,failedHost,failedPort,successorHost,successorPort)
	fmt.Println(sendMsg)
	// 发送DC消息
	sendDcMsg(sendMsg)
