...
func main()  {
	...
	if baseDir == "" || dataDir == "" {
		log.Println("baseDir 和 dataDir不能设置为空")
		return
	}

	// 安装主库
	fmt.Printf("%s开始部署主库%s%s",Green,remoteMasterAddr,Reset)
	installMysql(remoteMasterAddr)

	if remoteSlaveAddr != "" {
		slaveAddrs := strings.Split(remoteSlaveAddr,",")
		for _,slaveAddr :=  range slaveAddrs {
			fmt.Printf("%s开始部署从库%s%s",Green,slaveAddr,Reset)
			installMysql(slaveAddr)
			createRepl(slaveAddr,remoteMasterAddr)
		}
...
// 需要接受一个参数  ，否则使用了传入的参数
func createConfigFile(dataDirFull, remoteAddr string) (error , string){
	// 配置文件模板
	mysqlConfig := fmt.Sprintf(
		...
		port,dataDirFull,port,baseDir,dataDirFull,dataDirFull,dataDirFull,dataDirFull,lowerCaseTableNames,serverId,dataDirFull,dataDirFull,dataDirFull,dataDirFull,dataDirFull,readOnly,dataDirFull,dataDirFull,gtidMode,dataDirFull,dataDirFull,innodbBufferPoolSize,remoteAddr,port)
...
func installMysql(remoteAddr string){
	...
	if  err := checkAndCreatDir(client, dataDir); err != nil {
		log.Printf("创建目录%s",dataDir)
		return
	}
	fmt.Printf("创建目录%s成功",dataDir)

	fmt.Println(remoteAddr)

	// 生成serverID  取ID地址的后两位 加端口号
	serverId, err = genServerId(remoteAddr, port)
	if  err != nil {
		log.Println("生成ServerId失败")
		return
	}

	log.Printf("serverId 为 %d", serverId)

	// 创建子目录
	dataDirPort := fmt.Sprintf("mysql_%d", port)
	fmt.Println(dataDirPort)
	dataDirFull := filepath.Join(dataDir,dataDirPort)
	fmt.Println(dataDirFull)

	cmd = fmt.Sprintf("mkdir -p %s/{data,tmp,logs,run}",dataDirFull)
	fmt.Println(cmd)
	...
	if onlyInit {
		...
		// baseDir目录存在则退出
		if  dirFlag := checkDirExists(client, baseDir); dirFlag {
			log.Printf("目录%s已存在",baseDir)
			return
		}
		log.Printf("目录%s不存在",baseDir)

		// 目录不存在则解压并移动目录

		cmd = fmt.Sprintf("tar -zxf %s -C %s",remoteFile,remoteFilePath)
		fmt.Println(cmd)
		err,output = execCmd(client,cmd)
		fmt.Println(output)
		if err!=nil {

			log.Println("解压MySQL安装包失败")
			return
		}

		log.Println("解压MySQL安装包成功")

		packgeFile := strings.TrimSuffix(remoteFile,".tar.gz")
		fmt.Println(packgeFile)

		cmd = fmt.Sprintf("mv %s %s",packgeFile,baseDir)
		fmt.Println(cmd)
		if err,output = execCmd(client,cmd);err!=nil {
			log.Println(output)
			log.Println("移动MySQL安装包失败")
			return
		}

		log.Println("移动MySQL安装包成功")
	}

	// 创建用户与组
	cmd = "groupadd mysql && useradd mysql -g mysql"
	if err,_ = execCmd(client,cmd);err!= nil{
		log.Printf("创建用户和组失败 %s",err)
	}
	log.Println("创建用户和组成功")

	// 主库配置文件
	err, mysqlconfigFileName := createConfigFile(dataDirFull,remoteAddr)
	if err!=nil {
		log.Println("配置文件创建失败")
		return
	}

	log.Printf("配置文件%s创建成功",mysqlconfigFileName)

	// 拷贝配置文件至数据库服务器

	if err:= copyFile("root",mysqlconfigFileName,dataDirFull,remoteAddr);err!=nil{
		log.Println("配置文件传输失败")
		return
	}

	// 修改目录的用户
	cmd = fmt.Sprintf("chown -R mysql:mysql %s && chown -R mysql:mysql %s",baseDir,dataDirFull)
	if err, _ := execCmd(client,cmd) ; err != nil {
		log.Printf("目录用户属组修改失败！%s",err)
		return
	}

	log.Println("目录用户属组修改成功")

	// 初始化
	mysqlconfigFilePath := filepath.Join(dataDirFull,mysqlconfigFileName)
	if err:= initMysql(baseDir,mysqlconfigFilePath,client) ; err!=nil {
		log.Printf("初始化MySQL失败,%s",err)
		return
	}
	log.Println("初始化MySQL成功")

	// 启动
	if err:= startMysql(baseDir,mysqlconfigFilePath,client) ; err!=nil {
		log.Printf("启动MySQL失败,%s",err)
		return
	}
	log.Println("启动MySQL成功")

	// 通过select 1 验证MySQL启动成功
	testMysqlConn(remoteAddr)

	//创建复制账号
	//GRANT REPLICATION SLAVE, REPLICATION CLIENT ON *.* TO  'repl'@'10.%' identified by 'repl';
	cmd = fmt.Sprintf("%s/bin/mysql --socket=%s/run/mysql.sock --port=%d -e \"ALTER user 'root'@'localhost' IDENTIFIED BY '123456';\"",baseDir,dataDirFull,port)
	log.Println(cmd)

	cmd = fmt.Sprintf("%s/bin/mysql --socket=%s/run/mysql.sock --port=%d  -e \"GRANT REPLICATION SLAVE, REPLICATION CLIENT ON *.* TO  'repl'@'10.%%' identified by 'repl';\"",baseDir,dataDirFull,port)
	fmt.Println(cmd)
	execCmd(client,cmd)
	// 创建临时高权账号
	// grant all on *.*  to tmp_user@'10.%' identified by '123456';
	cmd = fmt.Sprintf("%s/bin/mysql --socket=%s/run/mysql.sock --port=%d   -e \"grant all on *.*  to tmp_user@'10.%%' identified by '123456';\"",baseDir,dataDirFull,port)
	fmt.Println(cmd)
	execCmd(client,cmd)

	//export PATH=$PATH:/usr/local/mysql/bin
	cmd = fmt.Sprintf("echo 'export PATH=$PATH:%s/bin' >> /etc/profile &&  source  /etc/profile",baseDir)
	log.Println(cmd)
	execCmd(client,cmd)
